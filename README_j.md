...
ESP32およびESP8266マイコン用の美しいWebインターフェースを持つ、シンプルでエレガントなOTA（Over-The-Air）アップデートライブラリです。

## 機能

- 使いやすい - わずか3行のコードで開始可能
- 美しいレスポンシブWebインターフェース
- HTTP Basic認証サポート
- モバイル対応のドラッグ&ドロップファイルアップロード
- ファームウェアとファイルシステムの両方のOTAアップデート
- リアルタイムアップロード進捗表示
- 包括的なエラーハンドリングとログ機能
- 開始、進捗、終了、エラーイベント用のカスタマイズ可能なコールバック
- アップデート成功後の自動再起動
...
ライブラリは`http://your_device_ip/update`でアクセス可能な美しいレスポンシブWebインターフェースを提供します。機能には以下が含まれます：
...
詳細なログを取得するためにデバッグモードを有効にしてください：

```cpp
ESP32FwUploader.setDebug(true);
```

これにより、OTAプロセスに関する詳細情報がSerialモニターに出力されます。

## サンプル

完全に動作するサンプルについては`examples`フォルダを参照してください：

- `BasicOTA` - 最小限の設定
- `AdvancedOTA` - 認証とコールバックを含む全機能サンプル
...
貢献を歓迎します！GitHubでプルリクエストの送信やイシューの作成をお気軽にどうぞ。
